...
function handleAnswerClick(selectedIndex, buttonEl) {
    ...
    if (isCorrect) {
        ...
        // --- NPC 情緒價值注入 ---
        const randomQuote = LogiNPC.correctQuotes[Math.floor(Math.random() * LogiNPC.correctQuotes.length)];
        subTextEl.innerHTML = `<span style="color:#fbbf24">👴 Logi:</span> 「${randomQuote}」`;
        // -----------------------
        ...
        // --- NPC 溫柔引導注入 ---
        const randomQuote = LogiNPC.wrongQuotes[Math.floor(Math.random() * LogiNPC.wrongQuotes.length)];
        subTextEl.innerHTML = `<span style="color:#f87171">👴 Logi:</span> 「${randomQuote}」`;
        // -----------------------
...
const LogiNPC = {
    ...
    wrongQuotes: [
        "別擔心，這題是在幫我們抓出小漏洞呢。",
        "挫折是成長的肥料，我們再挑戰一次！🌱",
        "沒關係，就算是偉大的數學家也會算錯，重要的是發現它。",
        "這題有點狡猾對吧？我們一起識破它的詭計。"
    ],
    // 完美過關（3星）的大型稱讚
    perfectFinish: (level) => `天啊！Level ${level} 完美通關！你現在的狀態簡直無敵，這份專注力足以挑戰更高的山峰！`,
    // 普通過關的大型稱讚
    passedFinish: (level) => `恭喜！你已經成功征服了 Level ${level}。每一道題都是一塊磚，你正在建造屬於自己的智慧城堡。`
};
